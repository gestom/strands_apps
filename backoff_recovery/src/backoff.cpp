...
void scanCallback (const sensor_msgs::LaserScan::ConstPtr& scan_msg)
{
		...
		float angle;
		misdetections++;
		d[0] = scan_msg->ranges[0];
		for (int i = 1; i <= filter; i++) d[i]+=d[i-1]+scan_msg->ranges[i];
		for (int i = filter; i < num_ranges; i++) d[i]+=d[i-1]-d[i-filter]+scan_msg->ranges[i];
		for (int i = 0; i < num_ranges; i++) printf("%.3f %3.f\n",scan_msg->ranges[i]*1.0,d[i]/filter);
